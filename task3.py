...
# --- CONFIGURAÇÃO DE CORES E VARIÁVEIS GLOBAIS DO MÓDULO ---
BRANCO = (255, 255, 255)
...
# Objetos de controle que precisam persistir entre os frames
fontes_carregadas = False
fonte_pqi = fonte_lista = fonte_med = fonte_negrito = fonte_titulo = fonte_gameover = None

estado_internal = "JOGANDO"
motivo_derrota = ""
cookies_visivel = False
cookies_clicado = False
tempo_surgimento_cookies = 0
...
erros = {
    0: pygame.Rect(175, 40, 350, 24),
    1: pygame.Rect(148, 43, 16, 16),
    2: pygame.Rect(620, 140, 145, 145),
    3: pygame.Rect(743, 128, 16, 16),
    5: pygame.Rect(300, 450, 200, 40),
}

encontrados = []
inicializado = False

def inicializar_task3():
    """Inicializa ou reinicia as variáveis da Task 3 com segurança."""
    global encontrados, cookies_visivel, cookies_clicado, tempo_surgimento_cookies, estado_internal, motivo_derrota
    global texto_email, texto_senha, campo_ativo, inicializado, fontes_carregadas
    global fonte_pqi, fonte_lista, fonte_med, fonte_negrito, fonte_titulo, fonte_gameover
...
def gerenciar_task3(tela, lista_eventos, LARGURA, ALTURA):
    global encontrados, cookies_visivel, cookies_clicado, tempo_surgimento_cookies, estado_internal, motivo_derrota
    global texto_email, texto_senha, campo_ativo, inicializado
    ...
    if estado_internal == "JOGANDO" and not cookies_visivel and not cookies_clicado and tempo_atual >= tempo_surgimento_cookies:
    ...
    for evento in lista_eventos:
        if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1:
            ...
            if estado_internal == "JOGANDO":
                ...
                # Condição de Vitória interna da Task
                if len(encontrados) == 6:
                    estado_internal = "VITORIA"
            
            elif estado_internal == "VITORIA":
                # Se o jogador já ganhou e clicou na tela, avança para a próxima task
                inicializado = False # Limpa para uma futura jogada
                return "FINAL_TASK"
        ...
        if evento.type == pygame.KEYDOWN:
            ...
            if estado_internal == "VITORIA" and evento.key == pygame.K_SPACE:
                inicializado = False
                return "FINAL_TASK"

            # Digitação nos inputs ativos
            if estado_internal == "JOGANDO" and campo_ativo is not None:
    ...
    # --- RENDERIZAÇÃO DA TELA DA TASK 3 ---
    ...
    if estado_internal == "VITORIA":
        ...
        msg_restart = fonte_pqi.render("Pressione [ESPAÇO] ou CLIQUE para ir para a Final Task", True, AMARELO_ALERTA)
        tela.blit(msg_vitoria, (40, 542))
        tela.blit(msg_restart, (40, 572))

    # Game Over da Task 3
    elif estado_internal == "GAME_OVER":
    ...
    # Se ainda estiver jogando ou em Game Over, mantém o estado como "TASK3"
    return "TASK3"